...
if (isset($_GET['query'])) {
  ...
  if ($result->num_rows > 0) {
      ...
      echo "<table><tr><th>ISBN</th><th>Title</th><th>Image</th><th>Publisher</th><th>Author</th><th>Number of Pages</th><th>Category</th><th>Summary</th><th>Keywords</th><th>Language</th><th>Copies in Library</th><th>Last Update</th></tr>";
      // output data of each row
      while ($row = $result->fetch_assoc()) {
          $image = base64_encode(file_get_contents('C:\xampp\htdocs\php_html\images\book_image.jpg'));
          $src = 'data:image/png;base64,' . $image;

          echo "<tr><td>".$row["ISBN"]."</td><td>".$row["book_title"]."</td><td><img src='".$src."' alt='Book cover' width='100'></td><td>".$row["publisher"]."</td><td>".$row["authors"]."</td><td>".$row["number_of_pages"]."</td><td>".$row["categories"]."</td><td>".$row["summary"]."</td><td>".$row["keywords"]."</td><td>".$row["book_language"]."</td><td>".$row["no_of_copies_in_library"]."</td><td>".$row["last_update"]."</td></tr>";
      }
  ...
  echo "<br>Search results for: " . htmlspecialchars($query);
}
?>